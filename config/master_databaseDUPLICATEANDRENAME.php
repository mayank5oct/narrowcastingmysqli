...
//-------------------------------------------
    function connect() {
        if (0 == $this->Link_ID_PREV) {
            $this->Link_ID_PREV = mysql_pconnect($this->Host, $this->User, $this->Password);
        }
        if (0 == $this->Link_ID_NEW_LOG) {
            $this->Link_ID_NEW_LOG = mysql_pconnect($this->Host_log, $this->User_log, $this->Password_log, true);
        }
        if (!$this->Link_ID_PREV)
            $this->halt("Link-ID == false, master_database host connect failed");
        if (!$this->Link_ID_NEW_LOG)
            $this->halt("Link-ID == false, master_database log host connect failed");

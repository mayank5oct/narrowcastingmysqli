...
//---------- code for fetching listing of carrousals-------------------
 $Query = "SELECT * FROM client_carrousel_listing ORDER BY name LIMIT 0,30";
 $conn=$db->query($Query);
 $count=mysql_num_rows($conn);
 $client_carrousel_list="";
 $sch_Query="";
 $b=0;
 $client_carrousel_list.="<table>";
 $client_carrousel_list .="<tr class='grid_header_top'><td colspan='6'><h2>Master carrousels</h2></td></tr>";
 $client_carrousel_list.="<tr class='grid_header'><td height='25' width='35%'>Naam</td>
	 
	  <td height='25' width='10%' align='center'>Bekijken</td>
  ...
  if($count==0) {
    $client_carrousel_list.="<tr><td colspan='5' align='center'><span style='margin:0px 0 0 0px;'>U heeft nog geen carrousel aangemaakt.</span></td></tr>";
    $client_carrousel_list.="</table>";
  } else {
    ...
    //$db->pr($result);
      $sch_Query = "select c.*, cs.* from client_schedule cs, clients c where find_in_set(c.id,cs.clients) and c.is_active=1 and cs.cid='$result[id]' order by cs.schedule asc";
	...
	if($result_sch['ipaddress']=='0') {
	 ...
	 $ip_address="";
      ...
      if($result['status']==0){
        if($eppc_status1==1) {
          $status="<img src='./img/$color_scheme/loader01.gif' border='0' width='20' height='20' >";	  
	 } else {
	  $status="<img src='./img/$color_scheme/loader01.gif' border='0' width='20' height='20' >";
	 }
       } else {
	...
	if($result['status']==0){
      ...
      $schedule="<a href=\"javascript:void(0)\" onclick='openclientsch(".$result['id'].")'><img src='./img/$color_scheme/inplannen.png' border='0' width='29' height='29' ></a>";
    ...
    // echo $multiscreen_status." @@@@@@@@@@@@ ";
     ...
     if($result['edit_status']==0){
      $edit_status= "<td align='center'> <a href='edit.php?id=".$result['id']."&status=$resstatus&is_client=1'><img src='./img/$color_scheme/edit03.png' border='0' width='30' height='28' align='center' /></a></td>";
      
     }else{
     $edit_status= "<td align='center'> <a href='edit.php?id=".$result['id']."&status=$resstatus&is_client=1' onclick=\"return confirm('Deze carrousel wordt op dit moment mogelijk door een andere gebruiker bewerkt. Het wordt afgeraden om een carrousel gelijktijdig door meerdere mensen te laten bewerken. Klik op Annuleren om terug te gaan of op OK om de carrousel te bewerken.');\"><img src='./img/edit_greyed.png' border='0' width='30' height='28' align='center' /></a></td>";
      }
     
     
     
     
      $client_carrousel_list.="<tr class=".$sty_class.">
      <td>".stripcslashes($result['name'])."</td>
     
      
    
      $edit_status
      <td align='center'><a href='javascript:void(0)' onclick=\"duplicateClientCarrousel('$id')\"><img src='./img/$color_scheme/duplicate_button.png' border='0' width='37' height='32' title='Duplicate Carrousel'></a></td>

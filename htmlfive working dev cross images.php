...
<div id="imagelayer" >
    <?php foreach($source_content as $key=>$val):?>
     <div style="display:none;"><img class="rotating-item"  id="img<?php echo $key; ?>" src="<?php echo $val; ?>" starttime="0"  displaytime="10" /></div>
     <?php endforeach; ?>
</div>
<?php
$inc=0;
...
$(document).ready(function(){
    ...
    $('.rotating-item').eq(1).fadeIn(initialFadeIn);
    var cnt = 1;
    var inc = 1;
        ...
        if(cnt < sequence_array.length){
            ...
            if(sequence_array[cnt]=='image'){
                 ...
                 $('#img0').attr("src",content_array[inc]);
                        ...
                        startImage(duration_array[inc], inc);
                    setTimeout(
                    function() 
                    {           
                        $("video").trigger("ended");
    ...
    function play_image(file,cnt){
        ...
        function startImage(dur,cnt) {
            console.log("cnt : "+cnt);
            var next = cnt+1;
         
           <?php if($enable_fade==1):?>
            //$('#img'+cnt).fadeIn(<?php echo $fade_in_time;?>);
           // $('#imagelayer').fadeIn(<?php echo $fade_in_time;?>);
           
            /* setTimeout(
                    function() 
                    {
                       // $('#img'+next).css('z-index',1);//move the next image up the pile
                      // $('#img'+next).fadeIn(3000).
                      //  $('#img'+cnt).fadeOut(3000)
                        $('#img'+cnt).fadeOut(20000).next().fadeIn(20000);
            ...
            */
            ...
            .appendTo('#imagelayer');
             ...
             <?php elseif($enable_fade==2):?>
             console.log("In else if !!");
                // $('.rotating-item').eq(cnt).fadeIn(initialFadeIn);
             $('.rotating-item').eq(cnt).fadeOut(1000);
                    //currentItem = cnt;
                ...
                $('.rotating-item').eq(cnt).fadeIn(fadeTime);
            <?php else: ?>
            $('#imagelayer').show();
             setTimeout(
                    function() 
